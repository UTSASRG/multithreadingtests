...
#define PRI 2
#define PYR 3
extern "C" void CalcAllFaceCentroids ( );
//================================================================================
...
{	EdgePtr		pECheck;
	...
	{
		pECheck = (EdgePtr)RetrieveListItem (pLEI, umEDGE, iLI);
...
{	NodePtr	pN;
	...
	for (nN = 0; nN < numN; nN++)
	{
		pN = (NodePtr)RetrieveListItem (pNL, umNODE, nN);	// get a Node in the list
...
{	
	...
	for (nF = 0; nF < numF; nF++)
	{
		pF2Test = (FacePtr)RetrieveListItem ( pLFI, umFACE, nF);		// get a Face from the list
...
EdgePtr FindEdgeFromSidesNodes( SidePtr pS )
{
	...
	for (nE = 0; nE < numE; nE++)
	{
		pE = (EdgePtr)RetrieveListItem ( pEL, umEDGE, nE);		// get a Face from the list
  ...
  numZ = Z_Total;					// retrieve the globally stored count of Zones
	...
	{	
		...
		//------------------ add Face to the Facelists of all of its Nodes
			for (nN = 0; nN < NodesInFace; nN++)
			{
				pN = (NodePtr)RetrieveListItem (pLNI, umNODE, nN);
    ...
    {  
      ...
      for( j=0;j<idSize;++j)                // loop over GLOBAL face ids for this tag
      {
        ...
        {											//					   looking for this COMM boundary Face
          ...
          {
            pN = (NodePtr)RetrieveListItem (pF->p1stNodeLI, umNODE, iN);
  ...
  DBF_LID = *pDBF_LID;
	...
	setNumBE ( 4*DBF_Total );	// this allocates boundary ELEMENT storage
		...
		{											//					   looking for this COMM boundary Face
			...
			{
				pC = (CornerPtr)RetrieveListItem (pF->p1stCornerLI, umCORNER, iC);
...
void C2K_nodecomm(int *nodecomm)	// COMM neighbor domain IDs and # boundary elements shared
{
	...
	DomainIDs           =(int*) malloc (ncomm*sizeof(int));
	NumBdyFacesOfDomain = (int*)malloc (ncomm*sizeof(int));
...
//									  one ORDERED Face Node at a time.		(ordered on GLOBAL Node ID)
	...
	BdyFaceGIDs = (int*)malloc (maxDomainBdyFaces*sizeof(int));	// to store GLOBAL IDs for boundary Faces
