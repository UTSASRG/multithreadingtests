	     ...
	     int f)
{
    ...
    {
      case FTW_F:
	fprintf(stderr, "%d: pqueue_put path %s\n", getpid(), path); 
...
worker(void *arg)
{
    ...
    {
	fprintf(stderr, "%d: getting the work %s\n", getpid(), path);
	scan_file(path);
	free(path);
    }
     ...
     char *argv[])
{
  ...
  EndOptions:
    ...
    aworkers = nworkers;
	fprintf(stderr, "create the worker thread %d************\n", nworkers);    
	...
	fprintf(stderr, "trying to pqueue close\n");
    pqueue_close(&pqb);
    ...
    pthread_mutex_lock(&aworker_lock);
    while (aworkers > 0) {
		fprintf(stderr, "hihi, i (%d) am going to sleep. aworkers %p with aworkers %d\n", getpid(), &aworkers, aworkers);
	pthread_cond_wait(&aworker_cv, &aworker_lock);
		fprintf(stderr, "hihi, i (%d) am going to wakeup. aworkers %p\n", getpid(), &aworkers);
	}
    pthread_mutex_unlock(&aworker_lock);
