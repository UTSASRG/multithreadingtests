...
void MeshVertexInit( MeshVertex *thissss)
{
  thissss->i = -1;
  thissss->j = -1;
  thissss->k = -1;
...
void MeshEdgeInit( MeshEdge *thissss)
{
  thissss->i = -1;
  thissss->j = -1;
  thissss->k = -1;
  thissss->meshVertexIds[0] = -1;
  thissss->meshVertexIds[1] = -1;
  thissss->baseNodeId = -1;
  thissss->numNodes = -1;
  thissss->baseEdgeId = -1;
  thissss->numEdges = -1;
  
}

void MeshFaceInit( MeshFace *thissss)
{
  thissss->i = -1;
  thissss->j = -1;
  thissss->k = -1;
  ...
  thissss->baseEdgeId = -1;
  thissss->numEdges = -1;
  thissss->baseFaceId = -1;
  thissss->numFaces = -1;
  thissss->alignment = -1;
  thissss->meshEdgeIds[0] = -1;
  thissss->meshEdgeIds[1] = -1;
  thissss->meshEdgeIds[2] = -1;
  thissss->meshEdgeIds[3] = -1;
}

void MeshCellInit(MeshCell *thissss)
{
  thissss->i = -1;
  thissss->j = -1;
  thissss->k = -1;
  thissss->meshFaceIds[0] = -1;
  thissss->meshFaceIds[1] = -1;
  thissss->meshFaceIds[2] = -1;
  thissss->meshFaceIds[3] = -1;
  thissss->meshFaceIds[4] = -1;
  thissss->meshFaceIds[5] = -1;
  thissss->baseZoneId = -1;
  thissss->numZones = -1;
  thissss->numStructuredZones = -1;
  thissss->baseNodeId = -1;
  thissss->numNodes = -1;
  thissss->numStructuredNodes = -1;
  thissss->baseFaceId = -1;
  thissss->numFaces =-1;
  thissss->baseEdgeId = -1;
  thissss->numEdges = -1;

  thissss->numberSubdivisions = 0;
  thissss->subdivisionIds = 0;
  
}



void MeshFaceContainerInit( MeshFaceContainer *thissss)
{
  int i;
  for(i=0;i<CMG_MAX_FACE_DEFINITIONS;++i)
      MeshFaceInit(&(thissss->faceList[i]));
}



void MeshVertexContainerInit( MeshVertexContainer *thissss)
{
  int i;
  for(i=0;i<CMG_MAX_VERTEX_DEFINITIONS;++i)
      MeshVertexInit(&(thissss->vertexList[i]));
  
}

void MeshEdgeContainerInit( MeshEdgeContainer *thissss)
{
  int i;
  for(i=0;i<CMG_MAX_EDGE_DEFINITIONS;++i)
      MeshEdgeInit(&(thissss->edgeList[i]));
  
}

void MeshCellContainerInit( MeshCellContainer *thissss)
{
  int i;
  for(i=0;i<CMG_MAX_BLOCK_DEFINITIONS;++i)
      MeshCellInit(&(thissss->cellList[i]));
  
} 


void MeshVertexPrint( const MeshVertex *thissss)
{
  if(thissss->i > -1)
      CMGDPRINT("MeshVertex i:%d j:%d k:%d, nodeId:%d\n",thissss->i,this->j,this->k,this->nodeId);
  else{
    CMGDPRINT(".\n");
  }
...
void MeshVertexContainerPrint( const MeshVertexContainer *thissss)
{
  ...
  for(i = 0; i<maxVertices;++i)
      MeshVertexPrint(&(thissss->vertexList[i]));
}


void MeshEdgePrint( const MeshEdge *thissss)
{
  if(thissss->i >-1)
      CMGDPRINT("MeshEdge i:%d j:%d k:%d alignment:%d\n\tbaseNodeId:%d numNodes:%d\n",thissss->i,this->j,this->k,this->alignment,this->baseNodeId,this->numNodes);
  else{
    CMGDPRINT(".\n");
  }
}

void MeshEdgeContainerPrint( const MeshEdgeContainer *thissss)
{
  ...
  for(i=0;i<maxEdges;++i)
      MeshEdgePrint(&(thissss->edgeList[i]));
}

void MeshFacePrint( const MeshFace *thissss)
{
  if(thissss->i>-1)
...
void MeshFaceContainerPrint( const MeshFaceContainer *thissss)
{
  ...
  for(i=0;i<maxFaces;++i)
      MeshFacePrint(&(thissss->faceList[i]));
}

void MeshCellPrint( const MeshCell *thissss)
{
  if(thissss->i>-1)
      CMGDPRINT("MeshCell i:%d j:%d k:%d\n\tbaseNodeId:%d numNodes:%d baseZoneId:%d numZones:%d\n",thissss->i,this->j,this->k,this->baseNodeId,this->numNodes,
                thissss->baseZoneId,this->numZones);
  else
      CMGDPRINT(".\n");
}

void MeshCellContainerPrint( const MeshCellContainer *thissss)
{
  ...
  for(i=0;i<maxCells;++i)
      MeshCellPrint(&(thissss->cellList[i]));
}
...
void createMeshTopology( )
{
  ...
  int *masterBlockArray =(int*) malloc(sizeof(int)*masterArraySize);
...
void createMeshCell( int i, int j, int k)
{
  extern SuperMeshSize sms;
    ...
    /*cells are straight forward,
      ...
      faces are associated with the cell, in thissss order
      i,i+1,j,j+1,k,k+1
    */
...
void createMeshCellVertices( int i, int j, int k)
{
  ...
  int nKVerts = sms.k+1; 
    ...
    /*lets create all the topology vertices for thissss block*/
...
int getMeshCellNodeSetSize( int cellId ,int getAll){
  ...
  if(getAll==1){
      /*We need to look at how many zones are unstructured and add that
        ...
        it has (same as number of subdivided zones).  We'll multiply thissss by
...
int getMeshCellEdgeNodeSetSize( int edgeId , int getAll){
    /*returns the nodes of thissss edge and its two associated vertices*/
  int maxEdges = getMaxEdges( );
  extern NumZones numZones;
                ...
                int **vertexList)
{
  ...
  *vertexList = (int*)malloc(sizeof(int) * size);
...
int getEdges(Range iRange, Range jRange, Range kRange,
             int **edgeList)
{
    /*Note if thissss is an edgeTag list range, edge tags are not supported
...
int getFaces(Range iRange, Range jRange, Range kRange,
             int **faceList)
{
    /*thissss will work for face tags as well as node tags*/
  ...
  *faceList =(int*) malloc(sizeof(int) * size);
             ...
             int **cellList)
{
    ...
    /*All tags are passed in vertex space, thissss makes things less confusing
  ...
  *cellList = (int*)malloc(sizeof(int) * size);
