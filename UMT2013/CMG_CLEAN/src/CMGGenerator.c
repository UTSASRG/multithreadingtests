...
{ 
  ...
  localZoneCheck = (int*)malloc(sizeof(int) *(iZones*jZones*kZones));
  ...
  cellContainer.cellList[cellContainerIndex].subdivisionIds =(int*) malloc(sizeof(int) * numberSubdividedZones);
...
void bcastinp( )
{
  ...
  for(i = 0;i<meshTags.numMeshTags;++i){
    ...
    if(myRank>0){
      meshTags.meshTags[i].meshTagType = (cMeshTagType)tag[0];
