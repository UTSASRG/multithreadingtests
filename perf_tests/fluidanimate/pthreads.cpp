...
void InitSim(char const *fileName, unsigned int threadnum)
{
	...
	int _cache_item_count = CACHE_SIZE/sizeof(pthread_spinlock_t);
	mutex = new pthread_spinlock_t *[numCells];
	for(int i = 0; i < numCells; ++i)
	{
		int n = (border[i] ? 16 : 1);
		mutex[i] = new pthread_spinlock_t[n*_cache_item_count];
		for(int j = 0; j < n; ++j)
			pthread_spin_init(&mutex[i][j*_cache_item_count], NULL);
	}
...
void CleanUpSim()
{
	...
	for(int i = 0; i < numCells; ++i)
	{
		int n = (border[i] ? 16 : 1);
		for(int j = 0; j < n; ++j)
			pthread_spin_destroy(&mutex[i][j*_cache_item_count]);
		delete[] mutex[i];
	}
			...
			for(int ix = grids[i].sx; ix < grids[i].ex; ++ix)
			{
				...
				for(int j = 0; j < np2; ++j)
				{
					...
					if(border[index2])
					{
						...
						np = __sync_fetch_and_add(&cnumPars[index2],1);	
						...
						for(int iparNeigh = 0; iparNeigh < numNeighPars; ++iparNeigh)
							if(&neigh.p[iparNeigh] < &cell.p[j])
							{
								...
								if(distSq < hSq)
								{
									...
									if(border[index])
									{
										pthread_spin_lock(&mutex[index][j*_cache_item_count]);
										cell.density[j] += tc;
										pthread_spin_unlock(&mutex[index][j*_cache_item_count]);
									}
									else
										cell.density[j] += tc;
									...
									{
										pthread_spin_lock(&mutex[indexNeigh][iparNeigh*_cache_item_count]);
										neigh.density[iparNeigh] += tc;
										pthread_spin_unlock(&mutex[indexNeigh][iparNeigh*_cache_item_count]);
									}
									else
										neigh.density[iparNeigh] += tc;
								}
