...
for i in `seq $time`
do
  startTime=`date +%s.%N | bc -l`
  /usr/bin/time ./SuOlsonTest problem1.cmg 64 2 64 8 8
  endTime=`date +%s.%N | bc -l`
  runningTime=`echo "$endTime"-$startTime | bc -l`
  echo "runningTime:$runningTime"
  #runningTime=`/usr/bin/time ./SuOlsonTest problem1.cmg 64 2 64 8 8  2>&1 | tee -a /dev/stderr | grep -o "[0-9]*elapsed" | grep -o "[0-9]*" | bc -l`
  #runningTime=`./amg2006 -r 30 30 30 | tee -a /dev/stderr | grep "wall clock time" | tail -n 1 | grep -o '[0-9]*\.[0-9]*' | bc -l`
