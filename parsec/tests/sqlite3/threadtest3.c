...
static void closedb_x(
  ...
  sqlite3_mprintf("close db %p", pDb);
...
static char *test_thread(int iTid, void *pArg){
  ...
  fprintf(stderr, "opendb\n");
	opendb(&err, &db, "test.db", 0);
  fprintf(stderr, "opendb finished, start to execute sql\n");
	nRow = execsql_i64(&err, &db, "SELECT * FROM t1");
  fprintf(stderr, "execute sql finished, close\n");
	closedb(&err, &db);
  fprintf(stderr, "closedb\n");

	if( nRow!=65536 ) test_error(&err, "Bad row count: %d", (int)nRow);
	print_and_free_err(&err);
	return 0;
}
static void test(int nMs){
	Error err = {0};
	Sqlite db = {0};
	Threadset threads = {0};
	...
	setstoptime(&err, nMs);
	launch_thread(&err, &threads, test_thread, 0);
	launch_thread(&err, &threads, test_thread, 0);
	launch_thread(&err, &threads, test_thread, 0);
	launch_thread(&err, &threads, test_thread, 0);
	launch_thread(&err, &threads, test_thread, 0);
...
int main(int argc, char **argv){
	struct ThreadTest {
		...
		{ dynamic_triggers, "dynamic_triggers", 20000 }, // fail

    { checkpoint_starvation_1, "checkpoint_starvation_1", 10000 }, //fail
  ...
  fprintf(stderr, "Line %d: Implanted memory buffer overflow at address %p\n", __LINE__, ptr);
  ptr = ptr+4;
  *ptr = 0;
#endif
